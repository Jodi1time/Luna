...
export function weightedCycleLength(starts, fallback) {
  ...
  return { length: model.p50, samples: model.samples }
}
...
export function cycleVariance(starts, fallback = 28) {
  const model = buildBayesianCycleModel(starts, fallback)
  const empiricalMean = model.samples
    ? model.intervals.reduce((sum, value) => sum + value, 0) / model.samples
    : null
  const empiricalStdDev = model.samples
    ? Math.sqrt(model.intervals.reduce((sum, value) => sum + ((value - empiricalMean) ** 2), 0) / model.samples)
    : null
  return {
    // Keep the historical contract: stdDev describes observed cycles.
    // The wider Bayesian predictive uncertainty is represented by range.
    stdDev: empiricalStdDev,
...
export function useCycle(store) {
  ...
  const cycleModel = buildBayesianCycleModel(starts, storedCycle)
  const variance = cycleVariance(starts, storedCycle)
