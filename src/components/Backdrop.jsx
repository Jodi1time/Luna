...
// ── Kind-specific tap effects ────────────────────────────────
// One-shot overlays positioned at the tap point. Each kind has its
// own reaction so the atmosphere feels like it knows it was touched.
// Position is `fixed` to client coordinates — the effect floats in
// the viewport regardless of scroll. Auto-cleared by the parent.

// 4 little stars radiating out, one per cardinal direction. Each
// gets its own --burst-rot inline so the keyframe rotates it before
// translating up — clean radial spread.
function SparkleEffect({ x, y, color }) {
  const rots = [0, 72, 144, 216, 288]
  return (
    <div className="tap-sparkle" style={{ left: x, top: y }} aria-hidden="true">
      {rots.map((rot, i) => (
        <i key={i} style={{ ['--burst-rot']: `${rot}deg`, animationDelay: `${i * 30}ms` }}>
...
function AuroraEffect({ x, y, color }) {
  return <div className="tap-aurora" style={{ left: x, top: y, ['--phase-color']: color }} aria-hidden="true" />
}

// 5 petals fly out radially, each rotating as it goes. Same trick as
// SparkleEffect — inline --burst-rot per child for the radial spread.
function PetalsEffect({ x, y, color }) {
  const rots = [0, 60, 120, 180, 240]
  return (
    <div className="tap-petals" style={{ left: x, top: y }} aria-hidden="true">
      {rots.map((rot, i) => (
        <i key={i} style={{ ['--burst-rot']: `${rot}deg`, animationDelay: `${i * 30}ms` }}>
          <svg width="10" height="16" viewBox="0 0 10 16">
            <ellipse cx="5" cy="8" rx="3" ry="7.5" fill={color} opacity="0.75" />
          </svg>
        </i>
      ))}
    </div>
...
// Bright core + expanding ring at the tap point. Two stacked overlays.
function StarFlashEffect({ x, y, color }) {
  return (
    <>
      <div className="tap-star-core" style={{ left: x, top: y, ['--phase-color']: color }} aria-hidden="true" />
      <div className="tap-star-ring" style={{ left: x, top: y, ['--phase-color']: color }} aria-hidden="true" />
    </>
  )
}

// Render the right effect for the current backdrop. Returns null when
// kind is 'blob' (handled inline on Home via the blob ripple/bloom
// overlays inside <Backdrop>'s children slot).
export function KindTapEffect({ kind, effect }) {
  if (!effect || !kind || kind === 'blob') return null
  const { x, y, color } = effect
  if (kind === 'moons')         return <SparkleEffect    x={x} y={y} color={color} />
  if (kind === 'aurora')        return <AuroraEffect     x={x} y={y} color={color} />
  if (kind === 'petals')        return <PetalsEffect     x={x} y={y} color={color} />
  if (kind === 'constellation') return <StarFlashEffect  x={x} y={y} color={color} />
  return null
}
