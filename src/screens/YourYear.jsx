...
export default function YourYear() {
  ...
  return (
    <Screen padBottom={40}>
      <div style={{ padding: '12px 22px 0', color: T.text }}>
        <Masthead issue="Your year with Luna" onBack={back} />
        <Eyebrow>A long look back — written for you</Eyebrow>
        ...
        <div>
          {narrative.sections.map((s, i) => (
            <section key={i} style={{ marginBottom: 22 }}>
              <div style={{ fontFamily: T.mono, fontSize: 10, letterSpacing: 1.4, fontWeight: 700, color: T.accent, marginBottom: 10 }}>
                {String(i + 1).padStart(2, '0')} · {s.heading.toUpperCase()}
              </div>
              {s.body.map((para, j) => (
                <ParaWithItalics key={j} text={para} />
              ))}
            </section>
          ))}
        </div>
