...
${flagged.length === 0
  ...
  : flagged.map((f) => `<div class="item"><div class="q">${f.q}</div><div class="a">${f.a}</div></div>`).join('')
}
<h2>All items reviewed</h2>
${RED_FLAGS.map((f) => `<div class="item" style="opacity:${answers[f.id]?1:.45}"><div class="q">${answers[f.id]?'✓ ':''} ${f.q}</div></div>`).join('')}
